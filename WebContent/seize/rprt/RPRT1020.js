 ...
 //        
 // ***********************************************************************************************/
corebase.wrapping("#RPRT1020", function($cbwrap, $cbfind, corebase){	
	// document ready
	...
	function fn_srch(){		
	    ...
	    corebase.selectList(source, "RPRT1020.SELECT_SVY_MKT_INFO", function(output, metadata) {	
	...
	function fn_exptFile(type) {	  		
        ...
        data.SELECT_EXSTCHK_MKT_INFO = corebase.Input("select", "RPRT1020.SELECT_EXSTCHK_MKT_INFO", source); // Marketing Existing Check
		option.data = data;				        
        corebase.ajax(option, function(output, metadata) {   				    	
            var exstChk = output.SELECT_EXSTCHK_MKT_INFO.metadata.resultCount;

            if(exstChk < 1) {
                CBMsg.error(cb.locMessage("msg.2258", "There is no existing survey marketing. Please check the marketing number you inputted."));
